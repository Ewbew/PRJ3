...
/*******************************************************************************
* Function Name: Clock_24MHz_Start
********************************************************************************
*
* Summary:
...
void Clock_24MHz_Start(void) 
{
    ...
    Clock_24MHz_CLKEN |= Clock_24MHz_CLKEN_MASK;
	Clock_24MHz_CLKSTBY |= Clock_24MHz_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_24MHz_Stop
********************************************************************************
*
* Summary:
...
void Clock_24MHz_Stop(void) 
{
    ...
    Clock_24MHz_CLKEN &= (uint8)(~Clock_24MHz_CLKEN_MASK);
	Clock_24MHz_CLKSTBY &= (uint8)(~Clock_24MHz_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_24MHz_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_24MHz_CLKEN & Clock_24MHz_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_24MHz_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_24MHz_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_24MHz__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_24MHz_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_24MHz_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_24MHz_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_24MHz_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_24MHz_GetSourceRegister();
    uint16 oldDivider = Clock_24MHz_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_24MHz_CLKEN & Clock_24MHz_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_24MHz_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_24MHz_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_24MHz_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_24MHz_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_24MHz_MOD_SRC |= modeBitMask & (uint8)Clock_24MHz_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_24MHz_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_24MHz_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_24MHz_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_24MHz_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_24MHz_MOD_SRC & (uint8)(Clock_24MHz_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_24MHz_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_24MHz_GetDividerRegister();
    uint8 oldSrc = Clock_24MHz_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_24MHz_MOD_SRC |= CYCLK_SSS;
        Clock_24MHz_MOD_SRC =
            (Clock_24MHz_MOD_SRC & (uint8)(~Clock_24MHz_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_24MHz_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_24MHz_MOD_SRC & Clock_24MHz_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_24MHz_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_24MHz_PHASE = clkPhase & Clock_24MHz_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_24MHz_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_24MHz_PHASE & Clock_24MHz_PHASE_MASK;
}
