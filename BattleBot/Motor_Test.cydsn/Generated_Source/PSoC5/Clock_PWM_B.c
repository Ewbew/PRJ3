...
/*******************************************************************************
* Function Name: Clock_PWM_B_Start
********************************************************************************
*
* Summary:
...
void Clock_PWM_B_Start(void) 
{
    ...
    Clock_PWM_B_CLKEN |= Clock_PWM_B_CLKEN_MASK;
	Clock_PWM_B_CLKSTBY |= Clock_PWM_B_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_PWM_B_Stop
********************************************************************************
*
* Summary:
...
void Clock_PWM_B_Stop(void) 
{
    ...
    Clock_PWM_B_CLKEN &= (uint8)(~Clock_PWM_B_CLKEN_MASK);
	Clock_PWM_B_CLKSTBY &= (uint8)(~Clock_PWM_B_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_PWM_B_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_PWM_B_CLKEN & Clock_PWM_B_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_PWM_B_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_PWM_B_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_PWM_B__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_PWM_B_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_PWM_B_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_PWM_B_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_PWM_B_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_PWM_B_GetSourceRegister();
    uint16 oldDivider = Clock_PWM_B_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_PWM_B_CLKEN & Clock_PWM_B_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_PWM_B_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_PWM_B_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_PWM_B_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_PWM_B_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_PWM_B_MOD_SRC |= modeBitMask & (uint8)Clock_PWM_B_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_PWM_B_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_PWM_B_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_PWM_B_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_PWM_B_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_PWM_B_MOD_SRC & (uint8)(Clock_PWM_B_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_PWM_B_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_PWM_B_GetDividerRegister();
    uint8 oldSrc = Clock_PWM_B_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_PWM_B_MOD_SRC |= CYCLK_SSS;
        Clock_PWM_B_MOD_SRC =
            (Clock_PWM_B_MOD_SRC & (uint8)(~Clock_PWM_B_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_PWM_B_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_PWM_B_MOD_SRC & Clock_PWM_B_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_PWM_B_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_PWM_B_PHASE = clkPhase & Clock_PWM_B_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_PWM_B_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_PWM_B_PHASE & Clock_PWM_B_PHASE_MASK;
}
