...
/*******************************************************************************
* Function Name: Clock_5kHz_Start
********************************************************************************
*
* Summary:
...
void Clock_5kHz_Start(void) 
{
    ...
    Clock_5kHz_CLKEN |= Clock_5kHz_CLKEN_MASK;
	Clock_5kHz_CLKSTBY |= Clock_5kHz_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_5kHz_Stop
********************************************************************************
*
* Summary:
...
void Clock_5kHz_Stop(void) 
{
    ...
    Clock_5kHz_CLKEN &= (uint8)(~Clock_5kHz_CLKEN_MASK);
	Clock_5kHz_CLKSTBY &= (uint8)(~Clock_5kHz_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_5kHz_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_5kHz_CLKEN & Clock_5kHz_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_5kHz_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_5kHz_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_5kHz__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_5kHz_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_5kHz_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_5kHz_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_5kHz_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_5kHz_GetSourceRegister();
    uint16 oldDivider = Clock_5kHz_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_5kHz_CLKEN & Clock_5kHz_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_5kHz_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_5kHz_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_5kHz_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_5kHz_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_5kHz_MOD_SRC |= modeBitMask & (uint8)Clock_5kHz_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_5kHz_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_5kHz_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_5kHz_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_5kHz_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_5kHz_MOD_SRC & (uint8)(Clock_5kHz_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_5kHz_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_5kHz_GetDividerRegister();
    uint8 oldSrc = Clock_5kHz_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_5kHz_MOD_SRC |= CYCLK_SSS;
        Clock_5kHz_MOD_SRC =
            (Clock_5kHz_MOD_SRC & (uint8)(~Clock_5kHz_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_5kHz_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_5kHz_MOD_SRC & Clock_5kHz_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_5kHz_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_5kHz_PHASE = clkPhase & Clock_5kHz_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_5kHz_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_5kHz_PHASE & Clock_5kHz_PHASE_MASK;
}
